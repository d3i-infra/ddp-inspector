...
def test_is_timestamp(test_timestamp: str, expected: bool) -> None:
    ...
    """
    assert (
        stringparse.is_timestamp(test_timestamp) is expected
    ), f"Timestamp: {test_timestamp} is not identified as bool: {expected}"


@pytest.mark.parametrize(
    "test_url, is_exact_url, expected",
    [
...
def test_has_url(test_url: str, is_exact_url: bool, expected: bool) -> None:
    ...
    """
    assert (
        stringparse.has_url(test_url, is_exact_url) is expected
    ), f"url: {test_url} in exact mode: {is_exact_url}, it should be bool: {expected}"


@pytest.mark.parametrize(
    "test_email, is_exact_email, expected",
    [
        ...
        (
            r"1234567890123456789012345678901234567890123456789012345678901234+x@example.co",
            True,
            True,
        ),
    ],
...
def test_has_email(test_email: str, is_exact_email: bool, expected: bool) -> None:
    ...
    """
    assert (
        stringparse.has_email(test_email, is_exact_email) is expected
